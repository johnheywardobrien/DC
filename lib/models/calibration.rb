class Calibration < ActiveRecord::Base
  ...
  validates :x_value, format: { with: /\A\.\d{4}\z/, message: "Please type valid input for X Value like .xxxx" }            
  validates :y_value, format: { with: /\A\.\d{4}\z/, message: "Please type valid input for Y Value like .xxxx" }            
